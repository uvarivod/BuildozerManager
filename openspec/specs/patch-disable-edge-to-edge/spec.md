# Patch Disable Edge-to-Edge

## Purpose

Provide a patch that disables Android's edge-to-edge display mode by setting `android:theme="@style/AppTheme.NoEdgeToEdge"` on the entrypoint activity and ensuring the `themes.xml` resource file exists in the build dist.
